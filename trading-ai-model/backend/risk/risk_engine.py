...
class DailyLossLimiter:
    """Tracks realised P&L and enforces the daily loss cap."""

    def __init__(self, max_loss_pct: float, account_size: float = 10_000.0) -> None:
    ...
    def is_limit_hit(self) -> tuple[bool, float]:
        limit = self._account_size * self._max_loss_pct
...
class RiskEngine:
    ...
    def __init__(self, account_size: float = 10_000.0) -> None:
        self._account_size = account_size
        ...
        self._daily_limiter = DailyLossLimiter(self._max_daily_loss, account_size)
        self._drawdown_monitor = DrawdownMonitor(self._max_drawdown, account_size)
        self._consecutive_losses = 0
        self._open_positions = 0
        self._gate = ProbabilityGate()

        logger.info(
            "RiskEngine: daily_loss=%.1f%% drawdown=%.1f%% max_contracts=%d min_rr=%.1f max_consec=%d",
    ...
    def approve(
        ...
        if daily_hit:
            rejections.append(
                f"Daily loss limit hit (>{self._max_daily_loss:.0%}). "
